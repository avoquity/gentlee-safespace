  ...
  return <motion.div initial={{
    ...
    delay: 0.2
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-4 md:gap-5 lg:gap-8">
        {testimonials.map((testimonial, index) => <TestimonialCard key={index} quote={testimonial.quote} name={testimonial.name} position={testimonial.position} delay={0.1 * index} />)}
      </div>
    </motion.div>;
};
